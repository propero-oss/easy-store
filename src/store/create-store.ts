...
export function createStore<T>(value: T, options?: StoreOptions): MutableStore<T> {
  const { notify, subscribe: sub, unsubscribe } = createSubscribable<[value: T, before?: T]>(options);
