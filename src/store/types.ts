...
export type StoreUpdate<T> = <Updater extends StoreUpdater<T>>(updater: Updater) => PromiseIf<Updater>;
...
export interface ReadOnlyStore<T = unknown> extends Subscribable<[value: T, before?: T]>, StoreReadAccessors<T> {}
...
export type DependencyValueTypes<T extends Subscribable[]> = {
  [Index in keyof T]: T[Index] extends Subscribable<infer Arr> ? Arr[0] : never;
};
export type DependencyHandler<T extends ReadOnlyStore[]> = (...values: DependencyValueTypes<T>) => void;
export type DeferredValueGenerator<T extends ReadOnlyStore[], R> = (...values: DependencyValueTypes<T>) => R;
